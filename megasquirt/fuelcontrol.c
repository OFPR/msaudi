...
ISR (TIMER0_COMPA_vect) {
  ...
  //start new injection, channel 1 начало впрыска
  if (inj_port1.status & _BV(scheduled)) {
    ...
    PORTB &= ~_BV(PWM_A);      // inject1 active форсунка открыта
    //PORTG |= _BV(FLYBACK_A);   // flyback enable гашение импулься включено vpizdu etu ebalu
  }

  //start new injection, channel 2 начало впрыска
  if (inj_port2.status & _BV(scheduled)) {
    inj_port2.status &= ~_BV(scheduled);
    inj_port2.status |= _BV(firing) | _BV(enabled);
    //PORTA |= _BV(LED_SQUIRT);  // sled on
    PORTB &= ~_BV(PWM_B);      // inject2 active форсунка открыта
    //PORTG |= _BV(FLYBACK_B);   // flyback enable гашение импулься включено
  }

  //check for end of injection, channel 1 проверка на конец фазы впрыска
  if (inj_port1.status & _BV(firing)) {
    if (inj_port1.pwrun >= inj_port1.pw) {
      inj_port1.status &= ~(_BV(firing) | _BV(enabled) | _BV(scheduled));
      TCCR1A &= ~( _BV(COM1A1) | _BV(COM1A0) );	// disable pwm полне отключение шим
      PORTB |= _BV(PWM_A);                      // inject1 deactive закрыть форсунку
      //PORTG &= ~_BV(FLYBACK_A);                 // flyback disable выключить гашение
    } else if (inj_port1.pwrun == inj_port1.pwm_delay) {
      OCR1A = inj_port1.pwm_dc * 10;  // pwm duty cycle, multiply by 10 to increase range
      TCCR1A |= _BV(COM1A1) | _BV(COM1A0);	// enable pwm Set OCnA/OCnB/OCnC on compare match (set output to high level)
    }
    inj_port1.pwrun++;
  }
  ...
  if (engine.status & _BV(running)) {
	...
	if ((engine.rpm_c>>8) >= 100) { 
	  ...
	  engine.status = 0; 
      /* установить флаги: датчик кислорода не готов, режим пуска */
	  ...
	  /* выключение топливнго насоса и клапана хоостого хода */
  ...
  if (++rtc.tick == 10) { // every ms
    static uint8_t	adc_clk;	//step_clk, adc_clk; vikinul wagovik
    ...
    if (++rtc.tsec == 100) {  // 1/10 second
      ...
      // stepper movement calculations sobstvenno obrabotwik wagovika
