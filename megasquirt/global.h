...
#define enabled   0 //впрыск
#define scheduled 1 //запланировано
#define firing    2 //firing хз что запуск? или сжигание топлива
...
#define crank   1  // cranking раскрутка
#define startw  2  // start warmup enrichment обогощение при старте
#define warmup  3  // warmup прогрев
#define tpsaen  4  // tps acceleration дросельная заслотка ускорение
#define tpsden  5  // tps deceleration дрсельная заслонка торможение
...
#define BATT  0  //Battery Voltage ADC Raw Reading - counts напряжение бортсети
#define EGO   1  //Exhaust Gas Oxygen ADC Raw Reading - counts датчик кислорода
#define TPS   2  //Throttle Position Sensor ADC Raw Reading - counts, represents 0 - 5 volts датчик положения дпосельной заслони
#define CLT   3  //Coolant Temperature ADC Raw Reading - counts (0 - 255) датчик температуры охлаждающей жидкости
#define MAT   4  //Manifold Air Temp ADC Raw Reading - counts (0 - 255) датчик температуры воздуха на впуске в фаренгеитах
#define MAP   5  //Manifold Absolute Pressure ADC Raw Reading - KPa (0 - 255) датчик обсалютного давления в коллекторе
#define BARO  6  //Barometer ADC Raw Reading - KPa (0 - 255) барометр????
...
#define ENGINE_STROKE   2  // 0:4-тактный, 		1:2-тактный
#define INJ_TYPE        3  // 0:форсунки в коллекторе, 1:одна форсунка у дросельной заслонки
...
//Port: C arduino
#define ADC_12V      0 // 12V voltage measurement напряжение бортовой сети
#define ADC_O2       1 // O2 sensor датчик остаточного кислорода
#define ADC_TPS      2 // throttle posistion sensor датчик положения дросельной заслонки потенциометр
#define ADC_CLT      3 // coolant датчик температуры охлаждающей жидкости
#define ADC_MAT      4 // manifold air temperature датчик температуры всасываемого воздуха
#define ADC_MAP      5 // manifold absolute pressure датчик давления во впускном коллекторе
